...
resource "aws_lb" "main" {
  ...
  access_logs {
    # Access logging disabled by default in broken workshop stack to avoid
    # requiring a pre-existing S3 bucket. Enable and set `logs_s3_bucket_name`
    # when a real bucket exists.
    enabled = false
